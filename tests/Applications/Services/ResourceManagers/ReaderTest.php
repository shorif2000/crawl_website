...
HTML;
    ...
    {
        $content = $this->createMock(FileInterface::class);
        $content->expects($this->any())
            ->method('fetch')
            ->willReturn($this->html);

        $reader = new Reader($content, new HtmlConverter());

        $this->assertEquals(1, $this->count($reader));
        $this->assertInstanceOf(VidexEntity::class, current($reader->read()));
    }
}
